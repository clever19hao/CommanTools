...
@property (nonatomic,copy) HBS_ViewHandle handle;

@end

@implementation HBS_View
{
...
- (id)init {
    ...
    return [[HBS_View alloc] initWithFrame:CGRectZero];
}
...
- (void)layoutSubviews {
    ...
    if (_contentAligment == ContentLeftRight) {
        ...
        CGSize tSize = [HBS_Tools getStrSize:_describeLabel.text font:_describeLabel.font containSize:CGSizeMake(self.bounds.size.width,10000)];
        if (tSize.height > self.frame.size.height - self.frame.size.width - _space) {
            tSize.height = self.frame.size.height - self.frame.size.width - _space;
        }
        
        _describeLabel.frame = CGRectMake(0, _foreImageSize.height + _space, self.bounds.size.width, tSize.height);
